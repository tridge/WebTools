...
(() => {
	...
	function pickSpacingMeters(mpp, latitude) {
            // choose 10^n meters so spacing ~150px
            const targetMeters = 150 * mpp;
            const logTarget = Math.log10(Math.max(1, targetMeters));
            const p10 = Math.pow(10, Math.floor(logTarget));

            // Apply latitude scaling to get true ground distance
            // Web Mercator stretches distances by 1/cos(latitude)
            const latRad = latitude * Math.PI / 180;
            const groundSpacing = p10 / Math.cos(latRad);

            // Debug output to verify we're getting powers of 10
            console.log(`Lat: ${latitude.toFixed(1)}°, Target: ${targetMeters.toFixed(1)}m, Web Mercator spacing: ${p10}m, Ground spacing: ${groundSpacing.toFixed(1)}m`);
	...
	function draw() {
            ...
            // Use the actual visible bounds for calculation, not expanded bounds
            const bounds = map.getBounds();
            const center = map.getCenter();

            // Expand bounds for drawing area but use original bounds for spacing calculation
            const latSpan = bounds.getNorth() - bounds.getSouth();
            const lngSpan = bounds.getEast() - bounds.getWest();

            // Create expanded bounds for drawing (to cover panning)
            const expandedBounds = L.latLngBounds([
		[bounds.getSouth() - latSpan * 2, bounds.getWest() - lngSpan * 2],
		[bounds.getNorth() + latSpan * 2, bounds.getEast() + lngSpan * 2]
            ]);

            // Calculate spacing based on the CENTER of the visible area, not expanded bounds
            const mpp = metersPerPixel(center.lat, map.getZoom());
            const spacingM = pickSpacingMeters(mpp, center.lat);

            const crs = map.options.crs;
            const nw = crs.project(expandedBounds.getNorthWest());
            const se = crs.project(expandedBounds.getSouthEast());
            const minX = Math.min(nw.x, se.x), maxX = Math.max(nw.x, se.x);
            const minY = Math.min(nw.y, se.y), maxY = Math.max(nw.y, se.y);

            // Get the layer point offset for proper positioning
            const topLeft = map.containerPointToLayerPoint([0, 0]);

            ctx.save();
            ctx.translate(-topLeft.x * dpr, -topLeft.y * dpr);
            ctx.lineWidth = 1;
            ctx.strokeStyle = 'rgba(255, 235, 59, 0.6)'; // light yellow
            ctx.beginPath();

            // Draw vertical lines - ensure we start at exact multiples of spacingM
		...
		ctx.lineTo(Math.round(p2.x * dpr) + 0.5, Math.round(p2.y * dpr));
            ...
            // Draw horizontal lines - ensure we start at exact multiples of spacingM
	...
	function updatePosition() {
            ...
            // Update canvas position to match layer positioning
            const topLeft = map.containerPointToLayerPoint([0, 0]);
	...
	function on() {
            ...
            // Listen to both move events (for repositioning) and end events (for redrawing)
